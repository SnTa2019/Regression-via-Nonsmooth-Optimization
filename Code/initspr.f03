...
MODULE initspr  ! Initialization of parameters.
    ...
    REAL(KIND=dp), SAVE :: &
        ...
        maxv = 2_dp, &                   ! Maximum value for pwl_points (need to be given by the user), 
                                         ! used with outfile3, othewise ignored.
        tol = 0.02_dp                    ! Stopping tolerance.
    ...
    REAL(KIND=dp), SAVE :: &
        ...
        eps = 0.010_dp, &                ! the epsilon used in SVM, change in init_par.
        CC = 1.00_dp, &                  ! the coefficient in front of maximum terms  (i.e. maximum from minimums)
...
MODULE initdbdc  ! Initialization of parameters for DBDC. There is, usually, no need to change these parameters.
    ...
    REAL(KIND=dp), PARAMETER :: &  
        user_m = 0.02_dp, &         ! The descent parameter. If user_m <= 0.0_dp .OR. user_m >= 1.0_dp
    ...
    INTEGER, PARAMETER :: &
        user_n = 2, & 
        user_size_b1 = MIN(user_n + 50,100), & ! The biggest possible size of the bundle B_1 
